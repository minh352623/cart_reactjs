...
const PhotoList = () => {
  const { photos, cartItems, likeList } = useGallery();
  //   console.log(cartItems);
  const countLike = photos?.filter((item) => item.isLike === true).length;
  return (
    <div className="py-10 px-5">
      <div className="flex items-center mb-5 gap-5">
        <span className="relative">
          ...
          <span className="absolute w-5 h-5 bg-pink-400 text-white rounded-full flex items-center justify-center top-[-8px] right-0 translate-x-2/4">
            {cartItems.length}
          </span>
        </span>
        <span className="relative">
...
const PhotoItem = ({ info: { url, isLike, id } }) => {
  const { toggleLikes, addToCart } = useGallery();
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group ">
      ...
      <span
        ...
        className="w-8 absolute z-10 top-5 right-5 cursor-pointer opacity-0  invisible transition-all duration-300 group-hover:opacity-100 group-hover:visible"
      >
        <svg
          width="42"
          height="38"
      ...
      <button
        ...
        className="opacity-0  invisible transition-all duration-300 group-hover:opacity-100 group-hover:visible px-6 py-3 text-sm font-semibold bg-white text-black rounded-lg shadow-md absolute bottom-5 left-2/4 -translate-x-2/4"
      >
        Add to cart
      </button>
    </div>
